...
while True:
    ...
    # print(buff)
    if buff:
        navidata=unpack('10if',buff)
        print(navidata)
        arw = int(navidata[0])
        meter.set_indicator_value(indic, int(arw)+50)
        if navidata[0] < 0:
            style1.set_text_color(lv.palette_darken(lv.PALETTE.BLUE, 4))
        elif navidata[0] > 0:
            style1.set_text_color(lv.palette_darken(lv.PALETTE.RED, 4))
        else:
            style1.set_text_color(lv.palette_darken(lv.PALETTE.YELLOW, 4))
        if navidata[9] == 0:
            label1.set_text("  %+4d ㎝"%navidata[1])
            label2.set_text("工程 :%d" %navidata[2])
            label3.set_text("幅 :%d㎝" %navidata[3])
            # label4.set_text("rev :%d" %navidata[4])
            label5.set_text("offset :%d㎝" %navidata[5])
            label6.set_text("圃場面積 :%4d㎡" %navidata[6])
            label7.set_text("作業面積 :%4d㎡" %navidata[7])
            if blf==0 :
                label8.set_text("→" ) 
            else :
                label8.set_text("←" ) 
